...
public final class SodiumReliefConfigScreen extends Screen {
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(ReliefTexts.button("export_benchmark"), button -> runtime.exportBenchmarkSnapshot("manual-export")).dimensions(width - 314, height - 28, 100, 20).build());
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        // Screen#render already draws the (blurred) background; calling renderBackground
        // again here triggers "Can only blur once per frame" on 1.21.11.
    ...
    private void buildOptions(ReliefConfig config) {
        switch (category) {
            ...
            case TOOLTIPS -> {
                ...
                addDrawableChild(toggle(1, "text_width_cache", () -> config.enableTextWidthCache, value -> config.enableTextWidthCache = value));
                addDrawableChild(toggle(2, "lazy_evaluation", () -> config.enableLazyEvaluation, value -> config.enableLazyEvaluation = value));
            }
            case HOVER -> {
