...
/**
 * Memoizes {@code TextRenderer.getWidth(String)} results. The width of a string is
 * deterministic for the current font generation, and the cache is cleared on every
 * client resource reload (which covers language and forced-unicode changes), so this
 * can never return a stale or wrong width. Targets the repeated per-frame measurement
 * of small strings such as item stack counts across all visible inventory slots.
 */
@Mixin(TextRenderer.class)
public abstract class FontWidthMixin {
    @WrapMethod(method = "getWidth(Ljava/lang/String;)I")
